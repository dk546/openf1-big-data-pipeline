...
def detect_full_duplicates(df: pd.DataFrame, table_name: str) -> pd.DataFrame:
    ...
    row_count = len(df)
    duplicate_count = _count_full_row_duplicates(df)
...
def detect_key_duplicates(
    ...
    subset = df[key_columns].dropna(how="any")
    dup_mask = subset.duplicated(keep=False)
    affected_rows = int(dup_mask.sum())
    duplicate_key_count = int(
        subset.duplicated(keep=False).sum() - subset.drop_duplicates(keep=False).shape[0]
    ) if affected_rows else 0
    # duplicate groups: keys appearing more than once
